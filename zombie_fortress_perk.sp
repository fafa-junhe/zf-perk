...
public OnPluginStart() {
  ...
  // Register cvars
  CreateConVar("sm_zf_version", PLUGIN_VERSION, "目前 Zombie Fortress Version", FCVAR_SPONLY | FCVAR_REPLICATED | FCVAR_NOTIFY);
...
//
////////////////////////////////////////////////////////////
public Action: hook_JoinTeam(client,
  const String: command[], argc) {
  decl String: cmd1[32];
...
public Action: hook_JoinClass(client,
  const String: command[], argc) {
  decl String: cmd1[32];
...
public Action: hook_VoiceMenu(client,
  const String: command[], argc) {
...
public Action: hook_zfTeamPref(client,
  const String: command[], argc) {
  decl String: cmd[32];
...
void SpawnEntity(char[] entity, float origin[3], float rotation[3] = {
  0.0,
  0.0,
  90.0
}) {
...
public Action: event_PlayerBuiltObject(Handle: event,
  ...
  new object_ = GetEventInt(event, "object");
...
handle_spawnState() {
  ...
  case ZF_SPAWNSTATE_REST: {
    ...
    if (zf_spawnRestCounter <= 0) {
      zf_spawnState = ZF_SPAWNSTATE_HUNGER;
      zf_spawnSurvivorsKilledCounter = 1;
      PrintToChatAll("\x05[ZF]\x01 僵尸饿了...(僵尸的重生时间缩短)");
      setTeamRespawnTime(zomTeam(), 8.0);
    }
  }
  ...
  case ZF_SPAWNSTATE_HUNGER: {
    if (zf_spawnSurvivorsKilledCounter <= 0) {
      ...
      PrintToChatAll("\x05[ZF]\x01 他们来了!(僵尸没有重生时间)");
  ...
  case ZF_SPAWNSTATE_FRENZY: {
    if (zf_spawnZombiesKilledCounter <= 0) {
      ...
      PrintToChatAll("\x05[ZF]\x01 我们安全了......暂时.(僵尸重生时间回归正常)");
...
//
////////////////////////////////////////////////////////////
public Sort_Preference(client1, client2,
  const array[], Handle: hndl) {
...
public help_printZFInfoChat(client) {
  if (client == 0) {
    PrintToChatAll("\x05[ZF]\x01 输入 \"!zf\" 打开僵尸模式菜单（查看帮助和选职业）!");
  } else {
    PrintToChatAll("\x05[ZF]\x01 输入 \"!zf\" 打开僵尸模式菜单（查看帮助和选职业）!");
  }
}
...
public panel_PrintMain(client) {
  ...
  SetPanelTitle(panel, "ZF模式菜单", false);
  ...
  DrawPanelItem(panel, "ZF模式帮助", 0);

  DrawPanelItem(panel, "ZF模式技能帮助", 0);
...
public panel_PrintPrefTeam(client) {
  ...
  if (prefGet(client, TeamPref) == ZF_TEAMPREF_NONE)
    DrawPanelItem(panel, "随机", ITEMDRAW_DISABLED);
  else
    DrawPanelItem(panel, "随机");

  if (prefGet(client, TeamPref) == ZF_TEAMPREF_SUR)
    DrawPanelItem(panel, "幸存者", ITEMDRAW_DISABLED);
  else
    DrawPanelItem(panel, "幸存者");

  if (prefGet(client, TeamPref) == ZF_TEAMPREF_ZOM)
    DrawPanelItem(panel, "僵尸", ITEMDRAW_DISABLED);
  else
...
public panel_PrintHelp(client) {
  ...
  SetPanelTitle(panel, "ZF模式帮助", false);
...
public panel_PrintHelpOverview(client) {
  new Handle: panel = CreatePanel();
  SetPanelTitle(panel, "Zombie Fortress 僵尸要塞 介绍", false);

  DrawPanelText(panel, "----------------------------------------");

  DrawPanelText(panel, "玩家在开局会被选为幸存者或僵尸.");

  DrawPanelText(panel, "幸存者必须活着撑到时间结束, 或者完成地图目标(占点等等).");

  DrawPanelText(panel, "僵尸则必须消灭所有人类, 或者完成地图目标.");

  DrawPanelText(panel, "幸存者死后会变为僵尸, 而僵尸可以无限重生.");
...
public panel_PrintHelpTeam(client, team) {
  ...
  if (team == surTeam()) {
    ...
    DrawPanelText(panel, "幸存者可使用的兵种: 士兵、火焰兵、");

    DrawPanelText(panel, "爆破手、工程师、医生和狙击手.");

    DrawPanelText(panel, "幸存者可以使用绝大部分武器.");
    ...
    DrawPanelText(panel, "僵尸可使用的兵种: 侦察兵、 机枪手和间谍.");

    DrawPanelText(panel, "僵尸的生命值会随时间缓慢恢复.");

    DrawPanelText(panel, "僵尸默认无法使用大部分枪械类武器.");

    DrawPanelText(panel, "(僵尸武器白名单制定中......)");
...
public panel_PrintHelpSurClass(client) {
  ...
  SetPanelTitle(panel, "幸存者兵种", false);
...
public panel_PrintHelpZomClass(client) {
  ...
  SetPanelTitle(panel, "僵尸兵种", false);
...
public panel_PrintClass(client, TFClassType: class) {
  ...
  case TFClass_Soldier: {
    SetPanelTitle(panel, "士兵 [幸存者]", false);

    DrawPanelText(panel, "----------------------------------------");

    DrawPanelText(panel, "移动速度由240下降到220.");

    DrawPanelText(panel, "----------------------------------------");
  }
  case TFClass_Pyro: {
    SetPanelTitle(panel, "火焰兵 [幸存者]", false);

    DrawPanelText(panel, "----------------------------------------");

    DrawPanelText(panel, "主武器备弹量更改为125.");

    DrawPanelText(panel, "移动速度由300下降到240.");

    DrawPanelText(panel, "----------------------------------------");
  }
  case TFClass_DemoMan: {
    SetPanelTitle(panel, "爆破手 [幸存者]");

    DrawPanelText(panel, "----------------------------------------");

    DrawPanelText(panel, "移动速度由280下降到260.");

    DrawPanelText(panel, "----------------------------------------");
  }
  case TFClass_Engineer: {
    SetPanelTitle(panel, "工程师 [幸存者]", false);

    DrawPanelText(panel, "----------------------------------------");

    DrawPanelText(panel, "建筑物不能升级,但可以修复");

    DrawPanelText(panel, "步哨枪的子弹只有60发");

    DrawPanelText(panel, "子弹会随时间减少,并且无法补充子弹.");

    DrawPanelText(panel, "弹药耗尽后，步哨就会自毁.");

    DrawPanelText(panel, "补给器的血量增加到250.");

    DrawPanelText(panel, "移动速度由300下降到270.");

    DrawPanelText(panel, "----------------------------------------");
  }
  case TFClass_Medic: {
    SetPanelTitle(panel, "医生 [幸存者]", false);

    DrawPanelText(panel, "----------------------------------------");

    DrawPanelText(panel, "针筒枪不需要换弹.");

    DrawPanelText(panel, "Uber充能速度更快,但持续时间较短.");

    DrawPanelText(panel, "超量治疗限制在原来血量的124%");

    DrawPanelText(panel, "超量治疗的血量衰减很慢.");

    DrawPanelText(panel, "移动速度由320下降到300.");

    DrawPanelText(panel, "----------------------------------------");
  }
  case TFClass_Sniper: {
    SetPanelTitle(panel, "狙击手 [幸存者]", false);

    DrawPanelText(panel, "----------------------------------------");

    DrawPanelText(panel, "移动速度由300下降到250.");
  ...
  case TFClass_Scout: {
    ...
    DrawPanelText(panel, "移动速度由400下降到350.");
  ...
  case TFClass_Heavy: {
    ...
    DrawPanelText(panel, "移动速度由230增加到250.");
  ...
  case TFClass_Spy: {
    ...
    DrawPanelText(panel, "无法使用死亡之铃."); 

    DrawPanelText(panel, "移动速度由300降低到280");
...
panel_PrintPerkHelp(client) {
  ...
  SetPanelTitle(panel, "ZF模式技能帮助", false);

  DrawPanelText(panel, "----------------------------------------");

  DrawPanelText(panel, "每个玩家都可以选择一个幸存者职业和僵尸职业");

  DrawPanelText(panel, "不同职业有不同的技能和增益减益,找到适合你的玩法吧!");
